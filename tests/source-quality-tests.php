...
$assert( str_contains( $files['includes/class-spf-purge.php']??'', "'spf_purge_stale_quarantine'" ), 'Stale destructive-purge quarantine blocker missing.' );
$assert( str_contains( $files['includes/class-spf-purge.php']??'', "'idempotent_replay'" ), 'Terminal purge replay evidence missing.' );
$assert( str_contains( $files['includes/class-spf-purge.php']??'', "'RENAME TABLE ' . implode" ), 'Atomic multi-table purge quarantine missing.' );
...
$assert( str_contains( $files['includes/class-spf-reconciler.php']??'', 'version-matched reversible reconciliation receipt' ), 'Version-bound owner reconciliation receipt missing.' );
