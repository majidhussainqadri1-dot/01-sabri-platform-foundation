...
s = p.read_text()
s = once(s, "\t\t\tpayload_json longtext NOT NULL,\n\t\t\tstatus varchar(32) NOT NULL DEFAULT 'pending',", "\t\t\tpayload_json longtext NOT NULL,\n\t\t\tprivacy_class varchar(32) NOT NULL DEFAULT 'internal',\n\t\t\tstatus varchar(32) NOT NULL DEFAULT 'pending',", "outbox privacy column")
s = once(s, "\t\t\t'privacy_holds' => array( 'hold_id','subject_type','subject_id','active' ),\n\t\t);", "\t\t\t'privacy_holds' => array( 'hold_id','subject_type','subject_id','active' ),\n\t\t\t'outbox' => array( 'event_id','event_version','payload_json','privacy_class','status','attempts','available_at' ),\n\t\t);", "outbox required privacy column")
verify_return = "\t\treturn empty( $missing ) ? true : new WP_Error( 'spf_schema_verification_failed', __( 'File 01 schema verification failed.', 'sabri-platform-foundation' ), array( 'defects' => $missing ) );\n\t}\n\n\tprivate static function seed_governance() {"
index_code = """\t\tif ( ! empty( $missing ) ) {
...
\tprivate static function seed_governance() {"""
s = once(s, verify_return, index_code, "required index verifier")
p.write_text(s)

p = Path("includes/class-spf-event-bus.php")
s = p.read_text()
s = once(s, "\tpublic static function publish( $event_name, $aggregate_type, $aggregate_id, array $payload, $version = 1, $dedupe_key = '' ) {", "\tpublic static function publish( $event_name, $aggregate_type, $aggregate_id, array $payload, $version = 1, $dedupe_key = '', $privacy_class = 'internal' ) {", "event publish signature")
s = once(s, "\t\tif ( ! $event_name || ! $aggregate_type || ! $aggregate_id || $version < 1 ) {\n\t\t\treturn new WP_Error( 'spf_invalid_event', __( 'Invalid event contract.', 'sabri-platform-foundation' ) );\n\t\t}\n\t\t$payload = self::sanitize_payload( $payload );", "\t\tif ( ! $event_name || ! $aggregate_type || ! $aggregate_id || $version < 1 ) {\n\t\t\treturn new WP_Error( 'spf_invalid_event', __( 'Invalid event contract.', 'sabri-platform-foundation' ) );\n\t\t}\n\t\t$privacy_class = sanitize_key( $privacy_class );\n\t\tif ( ! in_array( $privacy_class, array( 'public','internal','restricted','confidential','ephemeral' ), true ) ) {\n\t\t\treturn new WP_Error( 'spf_invalid_event_privacy_class', __( 'A valid event privacy classification is required.', 'sabri-platform-foundation' ) );\n\t\t}\n\t\t$payload = self::sanitize_payload( $payload );", "event privacy validation")
s = once(s, "\t\t\t\t'aggregate_id'=>$aggregate_id,'dedupe_key'=>$dedupe_key,'payload_json'=>$payload_json,\n\t\t\t\t'status'=>'pending','attempts'=>0,'available_at'=>$now,'created_at'=>$now,\n\t\t\t),\n\t\t\tarray( '%s','%s','%d','%s','%s','%s','%s','%s','%d','%s','%s' )", "\t\t\t\t'aggregate_id'=>$aggregate_id,'dedupe_key'=>$dedupe_key,'payload_json'=>$payload_json,'privacy_class'=>$privacy_class,\n\t\t\t\t'status'=>'pending','attempts'=>0,'available_at'=>$now,'created_at'=>$now,\n\t\t\t),\n\t\t\tarray( '%s','%s','%d','%s','%s','%s','%s','%s','%s','%d','%s','%s' )", "event privacy persistence")
p.write_text(s)
...
s = once(s, "array( 'purpose'=>$context['purpose']??'feature_flag' ) );\n\t\tif ( is_wp_error( $pre ) )", "array( 'purpose'=>$context['purpose']??'feature_flag','activation_evidence_hash'=>$activation_evidence_hash ) );\n\t\tif ( is_wp_error( $pre ) )", "feature activation precommit evidence hash")
s = once(s, "array( 'purpose'=>$context['purpose']??'feature_flag','enabled'=>empty($flag['enabled'])?0:1 ) );", "array( 'purpose'=>$context['purpose']??'feature_flag','enabled'=>empty($flag['enabled'])?0:1,'activation_evidence_hash'=>$activation_evidence_hash ) );", "feature activation success evidence hash")
p.write_text(s)

p = Path("qa/wp-runtime-smoke.php")
s = p.read_text()
s = once(s, "$assert( ! is_wp_error( $schema ), 'Schema verification failed: ' . ( is_wp_error( $schema ) ? $schema->get_error_message() : '' ) );", "$assert( ! is_wp_error( $schema ), 'Schema verification failed: ' . ( is_wp_error( $schema ) ? $schema->get_error_message() : '' ) );\n$assert( true === SPF_Installer::verify_required_indexes(), 'Required File 01 index integrity failed.' );", "runtime index assertion")
s = once(s, "$flag = SPF_Governance::set_flag( [ 'owner_module'=>'file-01','flag_key'=>'runtime_probe','environment'=>'all','enabled'=>true,'reason'=>'runtime test' ], [ 'purpose'=>'feature_flag' ] );", "$ungated_flag = SPF_Governance::set_flag( [ 'owner_module'=>'file-01','flag_key'=>'runtime_probe','environment'=>'all','enabled'=>true,'reason'=>'runtime test' ], [ 'purpose'=>'feature_flag' ] );\n$assert( is_wp_error( $ungated_flag ) && 'spf_evidence_unverified' === $ungated_flag->get_error_code(), 'Feature activation did not fail closed without readiness evidence.' );\nadd_filter( 'spf_verify_feature_activation_evidence', static fn() => [ 'verified'=>true,'migration_status'=>'ready','health_status'=>'pass','rollback_evidence'=>'ci-rollback-ready','gate_evidence'=>'ci-gate-proof','verifier'=>'CI runtime','expires_at'=>gmdate('c',time()+3600) ] );\n$flag = SPF_Governance::set_flag( [ 'owner_module'=>'file-01','flag_key'=>'runtime_probe','environment'=>'all','enabled'=>true,'reason'=>'runtime test' ], [ 'purpose'=>'feature_flag' ] );", "runtime feature activation assertion")
p.write_text(s)
